...
def create_case_session(tenant_id: str, created_by: str, case_id: Optional[str] = None, analyst_id: Optional[str] = None, senior_analyst_id: Optional[str] = None) -> CaseSession:
    """
    Create a new CaseSession (one per investigation).
    The case_id and timestamp are auto-generated by the schema if case_id is omitted.
    """
    if case_id:
        session = CaseSession(case_id=case_id, tenant_id=tenant_id, created_by=created_by, analyst_id=analyst_id, senior_analyst_id=senior_analyst_id)
    else:
        session = CaseSession(tenant_id=tenant_id, created_by=created_by, analyst_id=analyst_id, senior_analyst_id=senior_analyst_id)
    ...
    if _should_attempt_postgres():
        try:
            ...
            cur.execute(
                "INSERT INTO cases (case_id, tenant_id, created_by, status, analyst_id, senior_analyst_id) VALUES (%s, %s, %s, 'open', %s, %s) ON CONFLICT DO NOTHING;",
                (session.case_id, session.tenant_id, session.created_by, session.analyst_id, session.senior_analyst_id)
            )
            conn.commit()
            conn.close()
...
def list_cases(tenant_id: str) -> list[CaseSession]:
    ...
    if _should_attempt_postgres():
        try:
            ...
            cur = conn.cursor()
            cur.execute(
                "SELECT case_id, tenant_id, created_at, created_by, status, analyst_id, senior_analyst_id FROM cases WHERE tenant_id = %s ORDER BY created_at DESC;",
                (tenant_id,)
            )
            rows = cur.fetchall()
            conn.close()
            for row in rows:
                cases.append(CaseSession(
                    ...
                    created_by=row[3],
                    status=row[4],
                    analyst_id=row[5] if len(row) > 5 else None,
                    senior_analyst_id=row[6] if len(row) > 6 else None
                ))
        except Exception as e:
            logger.error("Failed to query cases: %s", e)
    return cases

def close_case(tenant_id: str, case_id: str) -> bool:
    """
    Updates the status of a CaseSession to 'closed' for the given tenant.
    """
    ...
    if _should_attempt_postgres():
        try:
            ...
            logger.error("Failed to close case: %s", e)
    return False
