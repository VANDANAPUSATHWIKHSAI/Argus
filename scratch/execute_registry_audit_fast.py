"""
ARGUS Registry Evidence Execution & Pipeline Audit (Fast Optimized Path)
=============================================================================
...
def execute_registry_pipeline_fast():
    ...
    print("=" * 80, flush=True)
    print("  ARGUS FORENSIC PIPELINE — REGISTRY EVIDENCE FAST AUDIT", flush=True)
    print(f"  Target Directory: {reg_dir}", flush=True)
    print(f"  Target Hives    : {', '.join(hive_files)}", flush=True)
    print("=" * 80, flush=True)

    start_time = time.time()
    tenant_id = "tenant-registry-fast"
    ...
    # PHASE 4: Atomic Entity Extraction (Deterministic Regex/IOC Extractor)
    print("\n--- PHASE 4: Atomic Entity Extraction (Fast Deterministic Path) ---", flush=True)
    extractor = ArtifactExtractor()
    extractor._pipeline = None # Bypass sequential GPU LLM/NER for structured registry paths
    t_p4_start = time.time()
    extracted_entities = extractor.extract_artifacts(normalized_artifacts, evidence_id=case_id)
    print(f"Atomic Entities Extracted: {len(extracted_entities):,} in {time.time() - t_p4_start:.2f}s", flush=True)
    ...
    fcr_records = fcr_engine.correlate(
        artifacts=normalized_artifacts + extracted_entities,
        allow_single_artifact=True
    )
    print(f"FCR Correlation Records: {len(fcr_records):,} in {time.time() - t_p5_start:.2f}s", flush=True)
    print("Phase 5 Status    : PASS", flush=True)

    # PHASE 6: Evidence Consolidation
    print("\n--- PHASE 6: Evidence Consolidation Engine ---", flush=True)
    t_p6_start = time.time()
    ...
    print(f"Consolidated UAIs: {len(consolidated_uai_records):,} in {time.time() - t_p6_start:.2f}s", flush=True)
    ...
    t_p7_start = time.time()
    fir_repo = FIRRepository()
    art_map = {art.artifact_id: art for art in (normalized_artifacts + extracted_entities)}
    ...
    print(f"Total FIR Findings Produced: {len(all_findings):,} in {time.time() - t_p7_start:.2f}s", flush=True)
    ...
    for finding in all_findings:
        res = gateway.sanitize_finding(finding)
        sanitized_contexts.append(res)
...
if __name__ == "__main__":
    execute_registry_pipeline_fast()
